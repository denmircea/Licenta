...
import { getDeliveryUserOrdersHistory } from '../api/ordersApi';
...
const DeliveriesScreen: React.FC = (props) => {
    ...
    useEffect(() => {
        if (isFocused) {
            const fetchOrders = async () => {
                const completedOrders = await getDeliveryUserOrdersHistory();
        ...
        <View style={{ flex: 1, backgroundColor: '#f9f9f9', padding: 16 }}>
            {orders.length > 0 ? (
                ...
                <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', marginTop: 40 }}>
                    <Text style={{ color: '#888', fontSize: 16 }}>No deliveries found.</Text>
                </View>
            )}
        </View>
    );
